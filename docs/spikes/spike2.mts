...
const p1 = await compress([{ role: "user", content: `summarize in 5 bullets for my team, keep the numbers\n\n${long}` }]);
...
const p2 = await compress([{ role: "user", content: `fix this error, under 5 bullets\n\n${rep.join("\n")}` }]);
...
const p3 = await compress([{ role: "user", content: `summarize\n\n${JSON.stringify(arr, null, 2)}` }]);
